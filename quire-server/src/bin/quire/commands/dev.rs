...
fn insert_runs(db: &rusqlite::Connection) -> Result<()> {
    ...
    let mut trans_stmt = db
        .prepare("INSERT INTO run_transitions (run_id, state, at_ms) VALUES (?1, ?2, ?3)")
        .into_diagnostic()?;

    for (id, state, sha, ref_name, pushed_at_ms, started_at_ms, finished_at_ms) in &runs {
        stmt.execute(params![id, repo, ref_name, sha, pushed_at_ms, workspace,])
            .into_diagnostic()?;

        // pending transition (queued_at_ms = pushed_at_ms for seed data)
        trans_stmt
            .execute(params![id, "pending", pushed_at_ms])
            .into_diagnostic()?;

        // active transition
        if let Some(s) = started_at_ms {
            trans_stmt
                .execute(params![id, "active", s])
                .into_diagnostic()?;
        }
