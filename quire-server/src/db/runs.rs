...
//!
//! Every function takes a `&Connection` — callers are responsible for
//! acquiring and releasing the connection. None of these functions open
...
pub fn insert_run(conn: &Connection, p: &NewRun<'_>) -> rusqlite::Result<()> {
    conn.execute(
...
pub fn get_run_meta(conn: &Connection, id: &str) -> rusqlite::Result<(String, String, i64)> {
    conn.query_row(
...
pub fn get_run_dispatched_at(conn: &Connection, id: &str) -> rusqlite::Result<Option<i64>> {
    conn.query_row(
...
pub fn get_run_resolved_at(conn: &Connection, id: &str) -> rusqlite::Result<Option<i64>> {
    conn.query_row(
...
pub fn get_run_outcome(conn: &Connection, id: &str) -> rusqlite::Result<Option<String>> {
    conn.query_row(
...
pub fn get_run_id_for_token(conn: &Connection, token: &str) -> rusqlite::Result<String> {
    conn.query_row(
...
pub fn get_run_bootstrap_data(
    conn: &Connection,
    id: &str,
) -> rusqlite::Result<Option<BootstrapData>> {
...
pub fn list_runs(conn: &Connection, repo: &str) -> rusqlite::Result<Vec<RunRow>> {
    conn.prepare(
...
pub fn set_run_dispatched(conn: &Connection, id: &str, now_ms: i64) -> rusqlite::Result<()> {
    conn.execute(
...
pub fn cancel_active_run(conn: &Connection, id: &str, now_ms: i64) -> rusqlite::Result<()> {
    conn.execute(
        "UPDATE runs SET \
...
pub fn fail_orphaned_runs(conn: &Connection, now_ms: i64) -> rusqlite::Result<usize> {
    conn.execute(
        "UPDATE runs SET \
            dispatched_at = COALESCE(dispatched_at, ?1), \
            resolved_at = COALESCE(resolved_at, ?2), \
...
pub fn count_runs(conn: &Connection) -> rusqlite::Result<i64> {
...
pub fn insert_seeded_run(conn: &Connection, r: &SeededRun<'_>) -> rusqlite::Result<()> {
    conn.execute(
...
#[cfg(test)]
pub fn get_run_token(conn: &Connection, id: &str) -> rusqlite::Result<Option<String>> {
    conn.query_row(
...
#[cfg(test)]
pub fn get_run_outcome_by_sha(conn: &Connection, sha: &str) -> rusqlite::Result<Option<String>> {
    conn.query_row(
...
#[cfg(test)]
pub fn count_unresolved_runs(conn: &Connection) -> rusqlite::Result<i64> {
    conn.query_row(
